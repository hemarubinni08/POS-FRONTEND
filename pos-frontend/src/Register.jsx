...
function Register() {
  ...
  const pwRules = [
    ...
    { label: "1 number", met: /[0-9]/.test(user.password) },
  ];
  ...
  const validate = () => {
    ...
    if (!/^[0-9]{10}$/.test(user.phoneNo))
      return "Phone number must be exactly 10 digits.";
    if (user.roles.length === 0) return "Please select at least one role.";
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#e8eaf0] flex items-center justify-center px-4 py-10">
        ...
        {/* Outer card */}
        <div className="fade-in flex w-full max-w-[900px] rounded-2xl overflow-hidden shadow-[0_8px_40px_rgba(0,0,0,0.18)]">

          {/* ── LEFT SIDEBAR ── */}
          <div className="w-[220px] min-w-[220px] bg-[#1e2433] flex flex-col justify-between p-7">

            {/* Brand */}
            <div>
              <div className="flex items-center gap-3 mb-10">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#3b82f6] to-[#6366f1] flex items-center justify-center text-lg">
                  🏪
                </div>
                <div>
                  <div className="text-white text-sm font-semibold leading-tight">POS System</div>
                  <div className="text-[#64748b] text-xs">Staff Portal</div>
                </div>
              </div>
                ...
                {steps.map((step) => (
                  <div key={step.num} className="flex items-start gap-3">
                    ...
                    <div>
                      <div className={
                        "text-sm font-medium " +
                        (step.active ? "text-white" : "text-[#4a5568]")
                      }>
                        {step.title}
                      </div>
                      <div className={
                        "text-xs " +
                        (step.active ? "text-[#94a3b8]" : "text-[#374151]")
                      }>
                        {step.sub}
                      </div>
                    </div>
          ...
          <div className="flex-1 bg-white p-10">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-5">
                ...
                {user.roles.length === 0 && (
                  <p className="text-xs text-[#94a3b8] mt-2">Select one or more roles</p>
                )}
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <button
                      ...
                      className="absolute top-1/2 right-3 -translate-y-1/2 text-[#94a3b8] bg-transparent border-none cursor-pointer text-base"
                    >
                      {showPw ? "🙈" : "👁"}
                    </button>
                  </div>
