...
const ProductRegistration = ({ onSuccess, onClose }) => {
  ...
  const validate = () => {
    ...
    if (!productData.supplierId || Number.isNaN(Number(productData.supplierId))) return "Valid supplier ID is required.";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post('/api/product/add', {
        ...productData,
        supplierId: Number.parseInt(productData.supplierId, 10)
      });
