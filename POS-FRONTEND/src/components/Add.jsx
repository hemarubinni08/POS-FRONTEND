...
function Add({ title, apiPath, extraFields = [], extraData: externalExtraData = {} }) {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const response = await apiFetch(`${API_BASE_URL}/api/${apiPath}/add`, {
                method: 'POST',
                body: JSON.stringify({ identifier, description, ...extraData, ...externalExtraData })
            });
            if (!response) return;
    ...
    return (
        <Layout>
            <div className='p-6'>
                <div className='mb-6'>
                    <h1 className='text-xl font-bold text-gray-800'>Add {title}</h1>
                            ...
                            <div key={field.key}>
                                ...
                                {field.type === 'custom' ? field.component
                                    : field.type === 'select' ? (
                                        ...
                                        <input type={field.type || 'text'} placeholder={`Enter ${field.label}`} onChange={(e) => handleExtraChange(field.key, e.target.value)} required={field.required || false} className={inputClass} />
                                    )}
                            </div>
                        ))}
