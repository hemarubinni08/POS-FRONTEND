...
const Sidebar = ({ isCollapsed = false }) => {
  ...
  style={{background: 'linear-gradient(180deg, #667eea'}}
>
  {isCollapsed ? (
    ...
    <h5 className="font-bold text-lg">POS System</h5>
      ...
      <nav className="flex-1 overflow-y-auto py-4">
        <ul className="flex flex-col gap-1 px-3">
          {loading ? (
            <li className="text-center text-sm opacity-75 mt-8 animate-pulse">Loading...</li>
          ) : error ? (
            <li className="text-center text-xs text-red-200 bg-red-500/20 rounded-lg p-3 mt-4">{error}</li>
          ) : (
            nodes.map((node) => {
