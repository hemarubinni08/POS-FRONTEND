...
const Register = () => {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-[#f0f4f8] to-[#e2e8f0] flex items-center justify-center p-5">
            <div className="bg-white w-full max-w-[440px] p-10 rounded-[24px] shadow-[0_20px_40px_rgba(0,0,0,0.1)] text-center">
                ...
                <h2 className="text-[#1f2937] text-2xl font-semibold m-0">Create Account</h2>
                <p className="text-[#6b7280] text-sm mb-[30px]">Join us to manage your business efficiently</p>

                {/* Status Messages */}
                {message && (
                    <div className={`p-[10px] rounded-lg text-[13px] mb-5 border ${
                ...
                <form onSubmit={handleRegister} className="text-left space-y-[18px]">
                    {/* Full Name */}
                    <div>
                        <label className="text-[13px] font-medium text-[#374151] ml-1 mb-1.5 block">Full Name</label>
  ...
  <div className="relative flex flex-col"> {/* Added flex-col for message spacing */}
    <div className="relative flex items-center">
      ...
      <input 
        ...
        />
                    ...
                    {/* Username */}
                    <div>
                        <label className="text-[13px] font-medium text-[#374151] ml-1 mb-1.5 block">Email</label>
                        <div className="relative flex items-center">
                            <EnvelopeIcon className="absolute left-3 w-5 text-[#9ca3af]" />
                            <input 
                                type="text" placeholder="user@gmail.com" required 
                                className="w-full py-3 pl-10 pr-3 border border-[#d1d5db] rounded-xl text-sm outline-none focus:border-[#2563eb] focus:ring-4 focus:ring-[#2563eb1a] transition-all text-gray-900"
                    ...
                    <div>
                        <label className="text-[13px] font-medium text-[#374151] ml-1 mb-1.5 block">Password</label>
                        <div className="relative flex items-center">
                            <LockClosedIcon className="absolute left-3 w-5 text-[#9ca3af]" />
                            <input 
                                type={showPassword ? "text" : "password"} 
                                placeholder="••••••••" required 
                                className="w-full py-3 pl-10 pr-12 border border-[#d1d5db] rounded-xl text-sm outline-none focus:border-[#2563eb] focus:ring-4 focus:ring-[#2563eb1a] transition-all text-gray-900"
...
<div className="relative" onClick={(e) => e.stopPropagation()}>
    <label className="text-[13px] font-medium text-[#374151] ml-1 mb-1.5 block">Role</label>
    <div className="relative flex items-center">
    ...
    {showRoleDropDown && (
        <div className="absolute z-[100] w-full bg-white border border-[#d1d5db] rounded-xl mt-2 shadow-xl max-h-52 overflow-y-auto overflow-x-hidden">
            {isSearchingRoles ? (
                ...
                <ul className="py-1">
                    {roles.map((role, idx) => (
                        <li 
                            ...
                            onClick={() => {
                                setFormData({...formData, roles: [role.identifier]}); // Wrap in array if backend expects list
                ...
                roleSearchTerm && <div className="p-3 text-sm text-gray-400">No roles found</div>
            )}
        </div>
    )}  
</div>
                ...
                <div className="mt-[25px] text-sm text-[#4b5563]">
                    Already have an account? <Link href="/login" className="text-[#2563eb] font-semibold hover:underline">Sign in here</Link>
                </div>
            </div>
        </div>
    );
