          ...
          {loading ? (
            ...
            <table className="w-full table-auto border-collapse">
              <thead className="sticky top-0 bg-slate-900 text-white text-xs font-semibold tracking-wider z-10">
                <tr>
                  {columns.map((col, i) => (
                    <th key={i} className="p-4 text-left whitespace-nowrap font-semibold">{col.label}</th>
                  ))}
              ...
              <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
                {filteredData.map((item, rowIdx) => (
                  <tr key={item.identifier || item.username || rowIdx} className="hover:bg-slate-50/80 transition-colors duration-100">
                    {columns.map((col, i) => (
                      <td key={i} className="p-4 whitespace-nowrap max-w-xs truncate">
                        {col.field === "status" && showStatus ? (
                          ...
                          <span className={col.field === "id" || col.field === "identifier" ? "font-mono text-slate-500 font-medium" : "text-slate-800"}>
                    ...
                    <td className="p-4 text-center whitespace-nowrap">
                      <div className="flex justify-center gap-1.5">
                        {/* ✅ KEY FIX: use editKey prop to navigate correctly */}
                        <button
                          onClick={() => navigate(`/${urlName}/edit/${item[editKey]}`)}
                          title="Edit row item"
